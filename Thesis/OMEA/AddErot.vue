...
export default {
  ...
  methods: {
    addE() {
      if(!this.title || !this.question || !this.selectedOption || !this.selectedOption2 || !this.date){
          this.show=true;
          setTimeout(()=>this.show=false,2500);
        }else{
        this.$emit('add-erot', this.title, this.question, this.Sans, this.selectedOption, this.date, this.selectedOption2);
        }
    },
        cNum(num) {
